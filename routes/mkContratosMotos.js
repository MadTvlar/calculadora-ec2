...
async function fetchMkContratosMotos(pool, sendLog, dataInicial, dataFinal) {
  ...
  for (const moto of dados) {
    ...
    const [rows] = await pool.query('SELECT empresa FROM microwork.contratos_motos WHERE contrato = ?',
      [moto.contrato]
    );

    if (!rows.length || rows[0].empresa !== moto.empresa) {
      const query = `
    ...
    `;
      ...
      try {
        ...
        sendLog(`Contrato ${moto.contrato}, inserido com sucesso.`);
      } catch (error) {
