...
router.post('/upload', upload.single('excelFile'), async (req, res) => {
  ...
  try {
    ...
    // Usa sempre a primeira aba encontrada no arquivo
    const worksheet = workbook.worksheets[0];

    if (!worksheet) {
      return res.status(400).send('Nenhuma aba encontrada na planilha enviada.');
    }
    ...
    // Pega o índice das colunas usando a segunda linha como cabeçalho
    const headerRow = worksheet.getRow(2);
    const colIndexes = {};

    headerRow.eachCell((cell, colNumber) => {

      // Normaliza o título para string pura
      const titulo = String(cell.value).trim();

      if (colMap[titulo]) {
        colIndexes[colMap[titulo]] = colNumber;
      }
    ...
    worksheet.eachRow((row, rowNumber) => {
      ...
      if (obj.empresa && obj.cnpj) {
        empresasComCnpj.add(JSON.stringify({
          empresa: obj.empresa,
          cnpj: obj.cnpj
        }));
      }
      ...
      if (
        obj.chassi &&
        obj.placa &&
        obj.data_emplacamento instanceof Date &&
        !isNaN(obj.data_emplacamento)
      ) {
        ...
        if (!dadosPorTabela[tabela]) {
          dadosPorTabela[tabela] = [];
        }
    ...
    for (const tabela in dadosPorTabela) {
      ...
      if (dados.length > 0) {
        ...
        const sql = `INSERT IGNORE INTO ${tabela}
          (cnpj, data_emplacamento, municipio, fabricante, empresa, modelo, ano, placa, uf, chassi)
          VALUES ?`;
    ...
    if (empresasComCnpj.size > 0) {
      ...
      const sqlCnpj = `INSERT IGNORE INTO tropa_azul.cnpj_empresa
        (empresa, cnpj) VALUES ?`;
